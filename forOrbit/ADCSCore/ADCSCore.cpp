...
Core::Core()
    : observer_(), refgen_(), controller_(), first_update(true), last_time(0.0)
{
}
...
AdcsOutput Core::update(const SensorData& sensors, const Command& cmd)
{
    ...
    // 2. Run observer
    Param::Vector17 states_hat = observer_.update(meas, sensors.unix_time, dt);

    // 3. Run reference generator with command
    auto ref_out = refgen_.update(states_hat, cmd, sensors.unix_time, dt);
    Param::Vector10 reference = ref_out.reference;
    Param::PointingMode internal_mode = ref_out.mode;

    // 4. Run controller
    auto ctrl_out = controller_.update(states_hat, reference, meas, internal_mode, dt);
...
void Core::reset() {
    ...
    first_update = true;
    last_time = static_cast<Param::Real>(0.0);
}
