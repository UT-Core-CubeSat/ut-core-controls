...
struct Vec {
    ...
    Vec(std::initializer_list<Real> init) {
    ...
    static Vec Constant(Real val) { return Vec(val); }

    static Vec UnitX() { 
        static_assert(N >= 1, "Vector too small for UnitX");
        Vec v; v.data[0] = static_cast<Real>(1.0); return v; 
    }
    static Vec UnitY() { 
        static_assert(N >= 2, "Vector too small for UnitY");
        Vec v; v.data[1] = static_cast<Real>(1.0); return v; 
    }
    static Vec UnitZ() { 
        static_assert(N >= 3, "Vector too small for UnitZ");
        Vec v; v.data[2] = static_cast<Real>(1.0); return v; 
    }
    ...
    // Check if all zeros
    bool isZero(Real tol = static_cast<Real>(1e-15)) const {
        for (int i = 0; i < N; ++i) {
...
struct Mat {
    ...
    Real& operator()(int r, int c) { return data[r * C + c]; }
    const Real& operator()(int r, int c) const { return data[r * C + c]; }
    ...
    static Mat Identity() {
        ...
        Mat m;
        for (int i = 0; i < R; ++i) m(i, i) = static_cast<Real>(1.0);
        return m;
    }
...
inline Mat<3, 3> inverse3x3(const Mat<3, 3>& m) {
    ...
    Real g = m(2,0), h = m(2,1), i = m(2,2);
    
    Real det = a*(e*i - f*h) - b*(d*i - f*g) + c*(d*h - e*g);
#ifdef DEBUG
    #include <cassert>
    if (std::abs(det) < static_cast<Real>(1e-15)) {
...
inline Mat<4, 4> inverse4x4(const Mat<4, 4>& m) {
    ...
    for (int col = 0; col < 4; ++col) {
        // Find pivot
        int pivot = col;
        Real maxVal = std::abs(aug(col, col));
        ...
        if (maxVal < static_cast<Real>(1e-15)) {
#ifdef DEBUG
            assert(false && "Matrix is singular, cannot invert");
#endif
            // Singular matrix
        ...
        // Scale pivot row
        Real scale = static_cast<Real>(1.0) / aug(col, col);
        for (int j = 0; j < 4; ++j) {
            aug(col, j) *= scale;
            result(col, j) *= scale;
        }
...
inline EigenDecomp4 jacobiEigen4x4(const Mat<4, 4>& A, int maxSweeps = 50, Real tol = static_cast<Real>(1e-12)) {
    Mat<4, 4> V = Mat<4, 4>::Identity();  // Eigenvector accumulator
    Mat<4, 4> D = A;                       // Working copy (will become diagonal)
    
    for (int sweep = 0; sweep < maxSweeps; ++sweep) {
        // Find largest off-diagonal element
        Real maxOffDiag = static_cast<Real>(0.0);
        int p = 0, q = 1;
        
        for (int i = 0; i < 4; ++i) {
            for (int j = i + 1; j < 4; ++j) {
                Real absVal = std::abs(D(i, j));
        ...
        Real theta;
        if (std::abs(aqq - app) < static_cast<Real>(1e-15)) {
            // Avoid division by zero when diagonal elements are equal
            theta = (apq > 0) ? PI / static_cast<Real>(4.0) : -PI / static_cast<Real>(4.0);
        } else {
            theta = static_cast<Real>(0.5) * std::atan2(static_cast<Real>(2.0) * apq, aqq - app);
        }
        ...
        D(p, q) = static_cast<Real>(0.0);
        D(q, p) = static_cast<Real>(0.0);
...
inline EigenResult4 maxEigenvector4x4Symmetric(const Mat<4, 4>& A) {
    ...
    if (result.eigenvector.hasNaN()) {
        result.eigenvector = Vec<4>{static_cast<Real>(1.0), static_cast<Real>(0.0), static_cast<Real>(0.0), static_cast<Real>(0.0)};
        result.eigenvalue = static_cast<Real>(0.0);
    }
    return result;
}
...
inline Mat<3, 3> skew(const Vec<3>& v) {
    ...
    S(1, 0) = v(2);   S(1, 1) = static_cast<Real>(0.0);    S(1, 2) = -v(0);
    S(2, 0) = -v(1);  S(2, 1) = v(0);   S(2, 2) = static_cast<Real>(0.0);
    return S;
}
