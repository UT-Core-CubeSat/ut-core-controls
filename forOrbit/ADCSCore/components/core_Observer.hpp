...
public: 
    ...
    StateVector update(const Param::Vector29& measurements, const TimeReal& t, const Scalar& dt);
...
private: 
    ...
    Quat quest_algorithm(const Param::TimeReal& JD, 
                         const Vector3& r_eci, 
                         const Vector3& b_sun, 
                         const Vector3& b_mag);
    ...
    Vector3 compute_r_mag(const Param::TimeReal& JD, 
                             const Vector3& r_eci);
    ...
    Param::TimeReal epoch_time;      // Unix timestamp at simulation start
    Param::TimeReal current_time;    // Current Unix timestamp (epoch + t)
    Vector3 omega_earth;
    bool use_star_tracker;
    bool use_magnetometer;
    bool use_sun_sensor;
    TimeReal last_star_update_time;
    TimeReal last_quest_update_time;  // Rate limit QUEST updates
    Scalar T_star;
